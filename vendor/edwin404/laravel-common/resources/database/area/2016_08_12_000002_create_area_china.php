...
class CreateAreaChina extends Migration
{
    /**
     * Run the migrations.
     *
    ...
    public function up()
    {
        Schema::create('area_china', function (Blueprint $table) {

            $table->increments('id');
            $table->timestamps();

            $table->integer('areaId')->nullable()->comment('地区ID');
            $table->integer('parentAreaId')->nullable()->comment('父ID');
            $table->integer('sort')->nullable()->comment('排序');
            $table->string('name', 50)->nullable()->comment('名称');

            $table->index(['areaId']);
            $table->index(['parentAreaId']);

        });

        $table = env('DB_PREFIX') . 'area_china';

        \Illuminate\Support\Facades\DB::statement("ALTER TABLE $table comment '中国地区列表'");
        ...
        foreach ($provinces as $p) {
            $data[] = ['areaId' => $p[0], 'name' => $p[1], 'parentAreaId' => $p[2], 'sort' => 9999,];
        }
        \Illuminate\Support\Facades\DB::table('area_china')->insert($data);
        $data = [];
        foreach ($cities as $p) {
            $data[] = ['areaId' => $p[0], 'name' => $p[1], 'parentAreaId' => $p[2], 'sort' => 9999,];
        }
