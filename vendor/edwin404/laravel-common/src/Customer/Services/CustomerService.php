...
namespace Edwin404\Customer\Services;
...
class CustomerService
{
    ...
    {
        return ModelHelper::load('customer_user', ['id' => $id]);
    }

    public function update($id, $data)
    {
        return ModelHelper::updateOne('customer_user', ['id' => $id], $data);
    }

    public function delete($id)
    {
        ModelHelper::delete('customer_user', ['id' => $id]);
    }

    public function loadByUsername($username)
    {
        return ModelHelper::load('customer_user', ['username' => $username]);
    }

    public function getCustomerByMemberUserId($memberUserId)
    {
        $m = ModelHelper::load('customer_user_slave', ['memberUserId' => $memberUserId]);
        if (empty($m)) {
            return null;
        }
        return ModelHelper::load('customer_user', ['id' => $m['userId']]);
    }
    ...
    public function uniqueCheck($type, $value, $ignoreUserId = 0)
    {
        ...
        $memberUser = ModelHelper::load('customer_user', [$type => $value]);
        if (empty ($memberUser)) {
     ...
     * @return array ['code'=>'0','msg'=>'ok','data'=>'<userId>']
     */
    public function register($username = '', $phone = '', $email = '', $password = '')
    {
        ...
        $memberUser = ModelHelper::add('customer_user', [
            'username' => $username,
            'email' => $email,
            'phone' => $phone,
            'password' => EncryptHelper::md5Encode($password, $passwordSalt),
    ...
    {
        $memberUser = ModelHelper::load('customer_user', ['id' => $userId]);
        if (empty($memberUser)) {
        ...
        ModelHelper::updateOne('customer_user', ['id' => $memberUser['id']], [
            'passwordSalt' => $passwordSalt,
    ...
    public function setAvatar($userId, $avatarData)
    {
        ...
        $imageBig = Image::make($avatarData)->resize(400, 400)->encode('jpg', 75);
        $imageMedium = Image::make($avatarData)->resize(200, 200)->encode('jpg', 75);
        $image = Image::make($avatarData)->resize(50, 50)->encode('jpg', 75);

        $retBig = DataFacade::upload($memberUser['id'], 'image', '大头像.jpg', $imageBig);
        if ($retBig['code']) {
            return Response::generate(-1, '头像存储失败（' . $retBig['msg'] . '）');
        }
        $retMedium = DataFacade::upload($memberUser['id'], 'image', '中头像.jpg', $imageMedium);
        if ($retMedium['code']) {
            DataFacade::deleteById($memberUser['id'], $retBig['data']['id']);
            if ($retBig['code']) {
                return Response::generate(-1, '头像存储失败（' . $retMedium['msg'] . '）');
            }
        }
        $ret = DataFacade::upload($memberUser['id'], 'image', '小头像.jpg', $image);
        if ($ret['code']) {
            DataFacade::deleteById($memberUser['id'], $retBig['data']['id']);
            DataFacade::deleteById($memberUser['id'], $retMedium['data']['id']);
            if ($retBig['code']) {
     ...
     * @param string $email
     * @param string $password
     * @return array ['code'=>'0','msg'=>'ok','data'=>customer_user]
     */
    public function login($username = '', $phone = '', $email = '', $password = '')
    {
        ...
        $memberUser = ModelHelper::load('customer_user', $where);
        if (empty($memberUser)) {
