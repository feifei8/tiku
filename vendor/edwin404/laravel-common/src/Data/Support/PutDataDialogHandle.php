...
class PutDataDialogHandle
{
    ...
    public function execute($category)
    {
        if (Request::isMethod('post')) {
            switch (Input::get('action')) {
                case 'save':
                    ...
                    if (empty($link)) {
                        return Response::send(-1, 'link empty');
                    }
                    $tempPath = $this->dataService->loadTempDataByPath($path);
                    if (empty($tempPath)) {
                        return Response::send(-1, 'path not found');
                    }

                    if (!preg_match('/^[a-zA-Z0-9_\\.\\/]+$/', $link)) {
                        return Response::send(-1, 'link path error');
                    }
                    ...
                    $content = file_get_contents($path);
                    if (empty($content)) {
                        return Response::send(-1, 'path data empty');
                    }

                    $path = DataService::DATA . '/' . $category . '/' . $link;
                    ...
                    $data = [];
                    $data['path'] = '/' . DataService::DATA . '/' . $category . '/' . $link;
        ...
        return view('common::data.putDataDialog', [
            'category' => $category,
        ]);
    }
